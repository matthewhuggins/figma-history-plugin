...
function applyCapAndDedupe(entry) {
...
async function loadHistory() {
    ...
    historyEntries = stored
        ...
        .map((entry) => {
        const entryType = entry.type === "selection" ? "selection" : "page";
        const viewportValue = entry.viewport &&
            typeof entry.viewport === "object" &&
            "zoom" in entry.viewport &&
            "center" in entry.viewport
            ? entry.viewport
            : undefined;
        const centerValue = viewportValue &&
...
function postHistoryToUi() {
    figma.ui.postMessage({
        type: "history-data",
        entries: historyEntries
    });
}
async function captureHistory(type) {
...
async function clearHistory(sendUiUpdate = true) {
    historyEntries = [];
    await figma.clientStorage.setAsync(storageKey, historyEntries);
    if (sendUiUpdate) {
        postHistoryToUi();
